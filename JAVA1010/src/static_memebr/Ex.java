...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * 1. 클래스 로딩 
		 * 	  -> Static 변수 및 메서드가 메모리에 로딩 
		 * 2. main()메서드 호출(실행) 
		 * 2-1. main() 메서드 실행시 stack 메모리 영역 생성 
		 * 3. 인스턴스 생성
		 * 	  -> 인스턴스 변수(멤버변수) 밑 메서드가 메모리에 로딩됨 
		 * 4. 인스턴스 메서드 호출(실행)
		 * 	  -> 로컬 변수 메모리에 로딩됨 
		 * 5. 결과 출력 
		 * 6. 프로그램 종료 
		 ...
		 *    인스턴스 생성과 상관없이 클래스가 로딩되는 시접에 메모리에 등록됨 
		...
		/*
		 * 인스턴스 멤버변수인 a 와 b는 인스턴스마다 생성되므로
		 ...
		 * 변경된 값이 두 인스턴스 모두 적용(공유) 
		 * -> 즉, 하나의 인스턴스에서 값을 변경하면 모든 인스턴스가 영향을 받음  
		 */
		
		System.out.println("---------------------------------------");
		
		s2.a = 100; //stack메모리에서 heap메모리를 가리켜 값 저장 후 method area 
		
		System.out.printf("s1.a: %d, s2.a: %d \n", s1.a, s2.a);
		
		// statics 멤버는 참조변수명 대신 클래스명만으로 접근 가능
		StaticMember.a = 500; // 바로 Method Area에 저장 
//		StaticMember.b = 500; // 컴파일 에러 발생 클래스명만으로 접근 가능
//		s2.b = 500; 
//		s1.b = 500;
...
class StaticMember{
	static int a = 10; // class variable(=static 멤버변수 = 정적 멤버변수)
